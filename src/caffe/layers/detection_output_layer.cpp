...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  // LOG(INFO) << "all_decode_bboxes, len=" << all_decode_bboxes.size();
  // LabelBBox box_ = all_decode_bboxes[0];
  //  for (map<int, vector<NormalizedBBox> >::iterator it = box_.begin();
  //        it != box_.end(); ++it) {

  //     int label = it->first;
  //     vector<NormalizedBBox> norm_boxes = it->second;
  //     LOG(INFO) << label << ":" << ", norm_boxes.size()=" << norm_boxes.size();

  //     for(int i=0; i < 100; i++){
  //         LOG(INFO) << "label:" << norm_boxes[i].label() << ",score**=" << norm_boxes[i].score() 
  //         << " ,xmin="<< norm_boxes[i].xmin() << ", ymin= " << norm_boxes[i].ymin() ;
  //     }

  //  }

  //  LOG(INFO) << "all_conf_scores:  len=" << all_conf_scores.size();
  //  map<int, vector<float> > score_ = all_conf_scores[0];
  //  for (map<int, vector<float> >::iterator it = score_.begin();
  //        it != score_.end(); ++it) {

  //     int label = it->first;
  //     vector<float > score_vector = it->second;
  //     LOG(INFO) << label << ":" << " score_vector.size()=" << score_vector.size() ;
  //  }
  

  
  // CHECK_EQ(1,2) << "num=" << num << ", all_conf_scores.size=" << all_conf_scores.size();
